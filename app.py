...
st.write("Use your webcam to capture an image or upload an image file to predict the emotion.")
...
with col1:
    st.header("Webcam")
    webrtc_ctx = webrtc_streamer(key="example", video_transformer_factory=EmotionClassifier)
    webcam_image = None
    if webrtc_ctx.video_transformer:
        if webrtc_ctx.video_transformer.prediction:
            st.write(f"Predicted Emotion from Webcam: {webrtc_ctx.video_transformer.prediction}")
...
if st.button("Classify Webcam Image") and webrtc_ctx.video_transformer:
    if webrtc_ctx.video_transformer.frame is not None:
        st.image(webrtc_ctx.video_transformer.frame, caption='Captured Image.', use_column_width=True)
